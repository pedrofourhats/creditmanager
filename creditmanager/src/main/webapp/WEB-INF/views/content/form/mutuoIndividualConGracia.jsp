		    ...
		    <h3>
               ...
               <p class="span-inline">
                 <b>PRIMERA: Monto del Préstamo</b>. El DEUDOR recibe en este acto del ACREEDOR la suma de $ {{addPoints(project.givenAmount)}} (PESOS {{numberToLetter(project.givenAmount)}}), mediante cheques del  Banco Credicoop Nº {{form.checkNumber}}, sirviendo el presente de eficaz recibo en legal forma, por lo cual el DEUDOR otorga a favor del ACREEDOR formal carta de adeudo.
                  <br>
                  <b>SEGUNDA: Amortización</b>. El  DEUDOR  se  obliga a pagar al ACREEDOR el total del préstamo que asciende a $ {{addPoints(project.givenAmount)}} (PESOS {{numberToLetter(project.givenAmount)}}) con más una tasa de interés mensual variable, que se establecerá sobre las bases estipuladas en la cláusula 9 del contrato de mutuo suscripto entre el FONCAP S.A. y el ACREEDOR, que el DEUDOR declara conocer y forma parte del presente contrato como Anexo I, en {{project.givenDeadline}} períodos mensuales y consecutivos, correspondientes a la deuda de capital,  otorgándose un plazo de gracia de {{project.requestedGracePeriod}} días para la devolución del capital a contar desde el momento de desembolso por parte de FONCAP, en el lugar y tiempo que indique el  ACREEDOR. En tal sentido, el DEUDOR comenzara a pagar las cuotas de los primeros servicios correspondientes a la atención de los intereses el día {{dateToLetters(project.formattedDeliveryDate)}} según consta en el cronograma de pago, entregado a la firma del presente contrato o al día hábil inmediato posterior en el caso que este fuera inhábil. A partir del {{dateToLetters(project.formattedDeliveryDate)}}, el DEUDOR comenzará a pagar conjuntamente los {{project.formattedDeliveryDate.substring(0,2)}} de cada mes las cuotas en concepto de capital e intereses de los meses sucesivos a la atención de los intereses o al día hábil inmediato posterior si estos fueran inhábiles.
                 <br>
                 ...
                 <br>
                 <b>DECIMA:</b> El DEUDOR ofrece en garantía los Contratos de Fianza <span data-ng-show="project.guarantors[0].isMale">del Sr</span><span data-ng-hide="project.guarantors[0].isMale">de la Sra</span>. <span class="uppercase">{{project.guarantors[0].surname}}, {{project.guarantors[0].name}}</span> con DNI Nº {{project.guarantors[0].identityNumber}} y <span data-ng-show="project.guarantors[1].isMale">del Sr</span><span data-ng-hide="project.guarantors[1].isMale">de la Sra</span>. <span class="uppercase">{{project.guarantors[1].surname}}, {{project.guarantors[1].name}}</span> con DNI Nº {{project.guarantors[1].identityNumber}}  emitidos en beneficio del ACREEDOR, respecto de las obligaciones por el asumidas en este Contrato de Mutuo, el que exhibe en este acto y cuya copia se agrega como Anexo II, y declara conocer y consentir, además, el contrato de mutuo suscripto el {{dateToLetters(project.formattedDeliveryDate)}}, entre el ACREEDOR y el FONCAP S.A., que se integra al presente contrato y se adjunta como anexo I. El DEUDOR, dará cumplimiento a las cargas y obligaciones contenidas en los referidos instrumentos que pesan sobre el ACREEDOR, así como se somete al régimen de causales de rescisión allí contenidas y a los efectos pactados, por su acción u omisión.
                 <br>
                 ...
                 <br>
                 <b>DÉCIMO SEGUNDA: Codeudores</b>. <span data-ng-show="project.guarantors[0].isMale">El Sr</span><span data-ng-hide="project.guarantors[0].isMale">La Sra</span>. <span class="uppercase">{{project.guarantors[0].surname}}, {{project.guarantors[0].name}}</span> con DNI Nº {{project.guarantors[0].identityNumber}} con domicilio en la calle <span class="uppercase">{{project.guarantors[0].address}} {{project.guarantors[0].addressNumber}} <span ng-show="project.guarantors[0].addressFloor">Piso {{project.guarantors[0].addressFloor}}</span> <span ng-show="project.guarantors[0].addressAppartment">Departamento {{project.guarantors[0].addressAppartment}}</span> </span> y <span data-ng-show="project.guarantors[1].isMale">el Sr</span><span data-ng-hide="project.guarantors[1].isMale">la Sra</span>. <span class="uppercase">{{project.guarantors[1].surname}}, {{project.guarantors[1].name}}</span> con DNI Nº {{project.guarantors[1].identityNumber}} con domicilio en la calle <span class="uppercase">{{project.guarantors[1].address}} {{project.guarantors[1].addressNumber}} <span ng-show="project.guarantors[1].addressFloor">Piso {{project.guarantors[1].addressFloor}}</span> <span ng-show="project.guarantors[1].addressAppartment">Departamento {{project.guarantors[1].addressAppartment}}</span> </span>, de la Ciudad de {{cityOnly(project.guarantors[1].locality)}}, Departamento de {{departmentOnly(project.guarantors[1].locality)}}, Provincia de ENTRE RÌOS, se constituyen en codeudores solidarios, fiadores lisos y llanos, principales pagadores del DEUDOR respecto del cumplimiento de todas y cada una de las obligaciones asumidas en este contrato frente al ACREEDOR, por lo cual este último podrá requerirle el cumplimiento de la totalidad de las obligaciones asumidas por el DEUDOR, no pudiendo invocar los beneficios de excusión  y/o división
                 <br>
