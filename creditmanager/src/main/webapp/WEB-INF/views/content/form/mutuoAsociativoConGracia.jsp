    ...
    <div class="panel panel-default">
		...
		<div class="panel-body" id="printableDiv">
                ...
                <p class="span-inline">
                  ...
                  <br>
                  <b>SEGUNDA: Amortización</b>. El  DEUDOR  se  obliga a pagar al ACREEDOR el total del préstamo que asciende a {{project.givenAmount}} ({{numberToLetter(project.givenAmount)}} PESOS) con más una tasa de interés mensual variable, que se establecerá sobre las bases estipuladas en la cláusula 9 del contrato de mutuo suscripto entre el FONCAP S.A. y el ACREEDOR, que el DEUDOR declara conocer y forma parte del presente contrato como Anexo I, en {{project.givenDeadline}} períodos mensuales y consecutivos, correspondientes a la deuda de capital,  otorgándose un plazo de gracia de {{project.requestedGracePeriod}} días para la devolución del capital a contar desde el momento de desembolso por parte de FONCAP, en el lugar y tiempo que indique el  ACREEDOR. En tal sentido, el DEUDOR comenzara a pagar las cuotas de los primeros servicios correspondientes a la atención de los intereses el día {{project.formattedDeliveryDate}} según consta en el cronograma de pago, entregado a la firma del presente contrato o al día hábil inmediato posterior en el caso que este fuera inhábil. A partir del {{project.formattedDeliveryDate}}, el DEUDOR comenzará a pagar conjuntamente los {{project.formattedDeliveryDate.substring(0,2)}} de cada mes las cuotas en concepto de capital e intereses de los meses sucesivos a la atención de los intereses o al día hábil inmediato posterior si estos fueran inhábiles.
                  <br>
                  ...
                  <br>
                  <b>DECIMA:</b> El DEUDOR ofrece en garantía los Contratos de Fianza del/la Sr./Sra. {{project.guarantors[0].name}} {{project.guarantors[0].surname}} con DNI Nº {{project.guarantors[0].identityNumber}} y del/la Sr./Sra. {{project.guarantors[1].name}} {{project.guarantors[1].surname}} con DNI Nº {{project.guarantors[1].identityNumber}}  emitidos en beneficio del ACREEDOR, respecto de las obligaciones por el asumidas en este Contrato de Mutuo, el que exhibe en este acto y cuya copia se agrega como Anexo II, y declara conocer y consentir, además, el contrato de mutuo suscripto el __________, entre el ACREEDOR y el FONCAP S.A., que se integra al presente contrato y se adjunta como anexo I. El DEUDOR, dará cumplimiento a las cargas y obligaciones contenidas en los referidos instrumentos que pesan sobre el ACREEDOR, así como se somete al régimen de causales de rescisión allí contenidas y a los efectos pactados, por su acción u omisión.
                  <br>
                  ...
                  <br>
                   <b>DÉCIMO SEGUNDA: Codeudores</b>. El/La Sr./Sra <span class="uppercase">{{project.guarantors[0].name}} {{project.guarantors[0].surname}}</span> con DNI Nº {{project.guarantors[0].identityNumber}} con domicilio en la calle {{project.guarantors[0].address}} y el/la Sr./Sra. <span class="uppercase">{{project.guarantors[1].name}} {{project.guarantors[1].surname}}</span> con DNI Nº {{project.guarantors[1].identityNumber}} con domicilio en la calle {{project.guarantors[1].address}}, de la Ciudad de {{cityOnly(project.guarantors[1].locality)}}, Departamento de {{departmentOnly(project.guarantors[1].locality)}}, Provincia de Entre Ríos, se constituyen en codeudores solidarios, fiadores lisos y llanos, principales pagadores del DEUDOR respecto del cumplimiento de todas y cada una de las obligaciones asumidas en este contrato frente al ACREEDOR, por lo cual este último podrá requerirle el cumplimiento de la totalidad de las obligaciones asumidas por el DEUDOR, no pudiendo invocar los beneficios de excusión  y/o división
                  <br>
                  <b>DECIMOTERCERA: Domicilio - Jurisdicción</b>. A todos los efectos del presente, el DEUDOR constituye domicilio con domicilio en la calle {{project.holders[0].address}} , Departamento de {{departmentOnly(project.guarantors[0].locality)}} y el ACREEDOR en Paraná 145 Piso 4º de la Ciudad Autónoma de Buenos Aires, en donde serán válidas todas las notificaciones judiciales y/o extrajudiciales que se cursen, y se someten a la jurisdicción y competencia de los Tribunales Ordinarios en lo Comercial de la Ciudad Autónoma de Buenos Aires, con exclusión de todo otro fuero o jurisdicción. En prueba de conformidad se suscriben tres ejemplares de un mismo tenor y a un sólo efecto en la Ciudad Autónoma de Buenos Aires, a los {{form.day}} días del mes de {{form.month}} del {{form.year}}.
                </p>
                <div class="row txtC signature">
                  <div class="col-sm-3">
                    Firma del DEUDOR 
                    <hr>
                    <span class="uppercase">{{project.holders[0].name}} {{project.holders[0].surname}}</span> 
                    <br>
